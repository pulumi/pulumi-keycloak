...
class UserFederationArgs:
    ...
    def _configure(
        ...
        if rdn_ldap_attribute is None and 'rdnLdapAttribute' in kwargs:
            rdn_ldap_attribute = kwargs['rdnLdapAttribute']
        if rdn_ldap_attribute is None:
            raise TypeError("Missing 'rdn_ldap_attribute' argument")
        if realm_id is None and 'realmId' in kwargs:
            realm_id = kwargs['realmId']
        if realm_id is None:
            raise TypeError("Missing 'realm_id' argument")
        if user_object_classes is None and 'userObjectClasses' in kwargs:
            user_object_classes = kwargs['userObjectClasses']
        if user_object_classes is None:
            raise TypeError("Missing 'user_object_classes' argument")
        if username_ldap_attribute is None and 'usernameLdapAttribute' in kwargs:
            username_ldap_attribute = kwargs['usernameLdapAttribute']
        if username_ldap_attribute is None:
            raise TypeError("Missing 'username_ldap_attribute' argument")
        if users_dn is None and 'usersDn' in kwargs:
            users_dn = kwargs['usersDn']
        if users_dn is None:
            raise TypeError("Missing 'users_dn' argument")
        if uuid_ldap_attribute is None and 'uuidLdapAttribute' in kwargs:
            uuid_ldap_attribute = kwargs['uuidLdapAttribute']
        if uuid_ldap_attribute is None:
            raise TypeError("Missing 'uuid_ldap_attribute' argument")
        if batch_size_for_sync is None and 'batchSizeForSync' in kwargs:
            batch_size_for_sync = kwargs['batchSizeForSync']
        if bind_credential is None and 'bindCredential' in kwargs:
            bind_credential = kwargs['bindCredential']
        if bind_dn is None and 'bindDn' in kwargs:
            bind_dn = kwargs['bindDn']
        if changed_sync_period is None and 'changedSyncPeriod' in kwargs:
            changed_sync_period = kwargs['changedSyncPeriod']
        if connection_timeout is None and 'connectionTimeout' in kwargs:
            connection_timeout = kwargs['connectionTimeout']
        if custom_user_search_filter is None and 'customUserSearchFilter' in kwargs:
            custom_user_search_filter = kwargs['customUserSearchFilter']
        if delete_default_mappers is None and 'deleteDefaultMappers' in kwargs:
            delete_default_mappers = kwargs['deleteDefaultMappers']
        if edit_mode is None and 'editMode' in kwargs:
            edit_mode = kwargs['editMode']
        if full_sync_period is None and 'fullSyncPeriod' in kwargs:
            full_sync_period = kwargs['fullSyncPeriod']
        if import_enabled is None and 'importEnabled' in kwargs:
            import_enabled = kwargs['importEnabled']
        if read_timeout is None and 'readTimeout' in kwargs:
            read_timeout = kwargs['readTimeout']
        if search_scope is None and 'searchScope' in kwargs:
            search_scope = kwargs['searchScope']
        if start_tls is None and 'startTls' in kwargs:
            start_tls = kwargs['startTls']
        if sync_registrations is None and 'syncRegistrations' in kwargs:
            sync_registrations = kwargs['syncRegistrations']
        if trust_email is None and 'trustEmail' in kwargs:
            trust_email = kwargs['trustEmail']
        if use_password_modify_extended_op is None and 'usePasswordModifyExtendedOp' in kwargs:
            use_password_modify_extended_op = kwargs['usePasswordModifyExtendedOp']
        if use_truststore_spi is None and 'useTruststoreSpi' in kwargs:
            use_truststore_spi = kwargs['useTruststoreSpi']
        if validate_password_policy is None and 'validatePasswordPolicy' in kwargs:
            validate_password_policy = kwargs['validatePasswordPolicy']
...
class UserFederation(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            cache = _utilities.configure(cache, UserFederationCacheArgs, True)
            ...
            kerberos = _utilities.configure(kerberos, UserFederationKerberosArgs, True)
